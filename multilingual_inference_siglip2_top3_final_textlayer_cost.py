...
def main(args, Height, Width):
    ...
    for i, data in enumerate(tbar):
        ...
        if lang_dic:
            ...
            for iii in range(num_sentences):
                ...
                # Use the final layer for merging
                current_sent = main_embs
                current_noun = noun_embs

                for layer in Model.model.text_model.encoder.layers:
    ...
    df.to_csv(f"./result_log/per_instance_multilingual_siglip2_{args.dataset}_{args.split}_{i}_final_textlayer_cost.csv", index=False)
    print("Saved per-instance CSV:", f"./result_log/per_instance_multilingual_siglip2_{args.dataset}_{args.split}_{i}_final_textlayer_cost.csv")
