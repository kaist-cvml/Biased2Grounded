...
def main(args, Height, Width):
            ...
            for iii in range(num_sentences):
                ...
                for i, layer in enumerate(layers[start_idx:], start=start_idx):
                    ...
                    for ll in range(len(main_embs)):
                        ...
                        layer_sent.append(sent_l.squeeze(0))  # (N, D)
                        layer_noun.append(noun_l.squeeze(0))
                ...
                for i, layer in enumerate(Model.model.text_model.encoder.layers):
                    ...
                    for l in range(len(current_sent)):
                        current_sent[l] = layer(current_sent[l], attention_mask=None)
                        current_noun[l] = layer(current_noun[l], attention_mask=None)
                ...
                for i in range(merge_layer, len(Model.model.text_model.encoder.layers)):
                    merged = Model.model.text_model.encoder.layers[i](merged, attention_mask=None)
    ...
    df.to_csv(f"./result_log/per_instance_multilingual_siglip2_{args.dataset}_{args.split}_{i}_centroid_textlayer_refined.csv", index=False)
    print("Saved per-instance CSV:", f"./result_log/per_instance_multilingual_siglip2_{args.dataset}_{args.split}_{i}_centroid_textlayer_refined.csv")

    f = open('./result_log/result_log_Multilingual_siglip2_centroid_textlayer_refined.txt', 'a')
    f.write(f'\n\n SigLIP22-base-patch16-384 Model'
...
if __name__ == "__main__":
    ...
    print(args.refer_data_root, 'naminie')
    with torch.no_grad():
